...
- **Nom Temporaire :** Lors de la soumission de `CreateCharacter.jsx`, le champ "name" du payload sera défini sur "Jeune Garou inconnu". Ce nom temporaire n'a pas besoin d'être unique (F6).
- **Rang par Défaut (F12) :** À la création en base, les nouveaux Garous auront un Rang initial de **1** (Cliath).
- **Validation d'Unicité du Nom (F6) :** Lors de la mise à jour finale de la fiche (quand le joueur remplace "Jeune Garou inconnu" par son nom et prénom définitifs), le backend NE DOIT PAS accepter un nom qui existe déjà pour un autre utilisateur. Le nom final DOIT être unique dans le système.
- **Discord Post Trigger (F2 & F5) :** Le webhook Discord (`create_character_thread`) qui se trouvait dans `create_character` de `character_service.py` sera déplacé ou appelé depuis la route de `PUT`/`POST` sur la mise à jour de la feuille (`update_character`), *uniquement* si c'est la première soumission de la fiche complète (ie. `old_character.discord_thread_id` est vide) ET que **TOUS les champs obligatoires** (Nom, Âge, Physique, etc.) sont remplis. Le message Discord envoyé devra inclure la fiche complète du joueur (Âge, Physique, Premier changement, Histoire), et non l'ancienne version restreinte.
...
- **Formulaire de la fiche (F3 & F4 & F7 & F8) :** Inspiré de la fiche Vampire (`web/src/modules/vampire/components/CharacterSheet.jsx`), le formulaire basculera entre un mode édition et un mode lecture.
  - La condition d'entrée dans le mode "Lecture forcée" (et la sortie du mode édition) exige que **tous les champs obligatoires !== null/vide**. Si le joueur efface son histoire (story = ""), il retombe en mode édition forcé bloquant la vue lecture.
  - S'il n'y a pas d'image (`image_url` vide), l'UI n'affichera tout simplement pas d'image en mode Lecture.
  - Le composant UI doit inclure l'input type file et la logique d'upload d'image vers Discord via le même endpoint utilisé par les vampires.
  - Tout affichage de Markdown (pour l'histoire complète) DOIT être protégé par un sanitizer de type `rehypeSanitize` pour éviter les failles XSS.
...
- [ ] Task 1: Mettre à jour le schéma de base de données WerewolfData
  ...
  - Action: Ajouter `age` (entier), `sex`, `physical_desc`, `mental_desc_pre`, `first_change`, `image_url` à la `dataclass` `WerewolfData` (Optional). Mettre à jour `werewolf_data_to_dict`, `create_werewolf_table` (ajouter les colonnes), `get_werewolf_data`, `create_werewolf_data`, `update_werewolf_data` pour gérer ces nouveaux champs.
...
- [ ] Task 2: Refactorer la création initiale backend
  ...
  - Action: Dans `create_character`, retirer l'appel à `create_character_thread` (Discord) et la sauvegarde de `discord_thread_id`. Le personnage est juste enregistré en base avec le nom fourni (qui viendra du front), et **forcer le Rank à 1 par défaut**.
...
- [ ] Task 4: Refactorer le Backend pour la mise à jour de fiche & Thread Discord
  ...
  - Action: Dans `update_character_handler` (ou là où est traité le `PUT /api/modules/werewolf/character`), gérer la mise à jour des nouveaux champs. **Implémenter une vérification d'unicité du `name` (sauf si c'est "Jeune Garou inconnu").** Ensuite, **si `old_character.discord_thread_id` est vide ET que TOUS les champs obligatoires sont remplis (Nom n'est plus "Jeune Garou inconnu", Âge, Sexe, Physique, Histoire présents)**, alors appeler `create_character_thread` et sauvegarder le `discord_thread_id`. Mettre à jour la payload envoyée à Discord pour inclure ces nouvelles données.
  - Notes: S'assurer que ça ne recrée pas un thread à chaque mise à jour de la fiche. Renvoyer une HTTP 400 si le nom définitif existe déjà.
...
- [ ] Task 5: Refonte du composant CharacterSheet Frontend (Onglet Ma Fiche)
  ...
  - Action: Remplacer le simple affichage d'histoire par deux modes: `isEditing` et `!isEditing`.
    - Édition: Formulaire avec champs `name` (Nom et Prénom), `age` (champ number, minimum 0, pas de max restrictif), `sex`, `physical_desc`, `mental_desc_pre`, `first_change`, `story`. **L'input du Nom doit afficher un placeholder vide et NE PAS pré-remplir "Jeune Garou inconnu" au joueur pour l'inviter à taper.** Inclure le composant d'upload d'image copié sur celui des vampires (`handleImageUpload`).
    - Lecture: Affichage structuré. Ne pas afficher d'image si `image_url` est vide. Markdown de l'histoire rendu en toute sécurité (`rehypeSanitize`). Le titre complet pourra temporairement afficher "Jeune Garou inconnu" sans traitement de faveur esthétique.
  - Notes: Copier/Adapter le pattern visuel de `vampire/components/CharacterSheet.jsx`. Le mode Lecture ne s'active que si tous les champs requis sont remplis.

### Acceptance Criteria

- [ ] AC 1: Given le joueur lance la création de personnage loup-garou, when il sélectionne Race, Auspice, Tribu (choix libre) et continue, then son personnage est créé avec le nom "Jeune Garou inconnu" SANS message envoyé sur Discord, et il est redirigé vers sa fiche.
- [ ] AC 2: Given un joueur a une fiche loup-garou sans TOUS les attributs complets (ex: histoire vide), when il va sur l'onglet "Ma Fiche", then il est bloqué sur le formulaire d'édition.
- [ ] AC 3: Given le joueur remplit intégralement son formulaire avec nom, âge, sexe, descriptions obligatoires et confirme, then les données sont sauvegardées en base et la vue bascule en mode Lecture. S'il n'a pas mis d'image, l'UI n'en affiche pas.
- [ ] AC 4: Given c'est la première soumission **complète** de la fiche du joueur, when le backend sauvegarde les informations, then un thread Discord est créé avec les **détails complets de la nouvelle fiche** du personnage.
